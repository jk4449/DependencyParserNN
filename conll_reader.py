...
class DependencyStructure(object):
    ...
    def __init__(self):
        self.deprels = {} #deprel = dependencyEdge object. DependencyStructure.deprels[index] = DependencyEdge. dict
        self.root = None
    ...
    def words(self):
        return [None]+[x.word for (i,x) in self.deprels.items()] #words[index] = word. array,  NOT dictionary (?)
    
    def pos(self):
        return [None]+[x.pos for (i,x) in self.deprels.items()] #pos[index] = pos. array,  NOT dictionary
...
def conll_reader(input_file):
    ...
    while True:
        ...
        if not line and current_deps:
            ...
            if not line:  
                break
        current_deps.add_deprel(parse_conll_relation(line))  #returns dependency object dict, parent to child dict, and root.
