...
def get_training_instances(dep_structure):
    ...
    deprels = dep_structure.deprels #deprel dictionary
...
class FeatureExtractor(object):
    ...
    def __init__(self, word_vocab_file, pos_vocab_file):
        self.word_vocab = self.read_vocab(word_vocab_file)   #word_vocab[word] = index     
    ...
    def word_to_index(self, w, words, pos):
        if w == 0:
            w = self.word_vocab["<ROOT>"]
        elif pos[w] == "NNP" or pos[w] == "CD" or pos[w] == "UNK":
            w = self.word_vocab["<" + pos[w] + ">"]
        else:
            w = words[w].lower()
            if w in self.word_vocab:
                w = self.word_vocab[w]
            else:
                w = self.word_vocab["<UNK>"]
        return w
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for i in range(3):
            if state.stack:
                w = state.stack.pop()
                rep.append(self.word_to_index(w, words, pos))
            else:
                rep.append(self.word_vocab["<NULL>"])
        for i in range(3):
            if len(state.buffer) == 1 and words[state.buffer[-1]] == None:
                rep.append(self.word_vocab["<NULL>"])
                state.buffer.pop()
            elif state.buffer:
                w = state.buffer.pop()
    ...
    def get_output_representation(self, output_pair): 
        rep = keras.utils.to_categorical(self.output_labels.get(output_pair), num_classes = 91)
        return rep
